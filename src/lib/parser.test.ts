...
describe("parseCommand — launch basics", () => {
  ...
  it("returns null for just @mention", () => { expect(parseCommand("@Ringdotfun")).toBeNull(); });
  it("returns null for launch without ticker", () => { expect(parseCommand("@Ringdotfun launch something")).toBeNull(); });
  it("returns null for launch without $", () => { expect(parseCommand("@Ringdotfun launch BULL Name")).toBeNull(); });
});

describe("parseCommand — pvp mode", () => {
  it("detects pvp keyword", () => {
    const r = parseCommand("@Ringdotfun launch $FIGHT Fighter pvp");
    if (r?.kind === "launch") expect(r.mode).toBe("pvp");
  });
  it("detects PVP uppercase", () => {
    const r = parseCommand("@Ringdotfun launch $X Coin PVP");
    if (r?.kind === "launch") expect(r.mode).toBe("pvp");
  });
  it("detects pvp in separate line", () => {
    const r = parseCommand("@Ringdotfun launch $X Coin\npvp");
    if (r?.kind === "launch") expect(r.mode).toBe("pvp");
  });
...
describe("parseCommand — bull", () => {
  it("parses bull", () => { expect(parseCommand("@Ringdotfun bull")).toEqual({ kind: "bull" }); });
  it("parses with whitespace", () => { expect(parseCommand("  @Ringdotfun   bull  ")).toEqual({ kind: "bull" }); });
  it("parses case insensitive", () => { expect(parseCommand("@Ringdotfun BULL")).toEqual({ kind: "bull" }); });
  it("parses Bull mixed case", () => { expect(parseCommand("@Ringdotfun Bull")).toEqual({ kind: "bull" }); });
  it("does not match bull with extra text", () => { expect(parseCommand("@Ringdotfun bull please")).toBeNull(); });
});

describe("parseCommand — combined fields", () => {
  it("parses all fields together", () => {
    const r = parseCommand("@Ringdotfun launch $MEGA Mega Coin pvp\nto: @vitalik\nsplit: 60/10/25/5\nfee: 0.8\nammfee: 0.5\nliq: 70\nimg: https://img.com/a.png");
    expect(r?.kind).toBe("launch");
    if (r?.kind === "launch") {
